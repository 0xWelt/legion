...
- **缺点**：文本以整段返回，不是逐字流式；CLI 不暴露 thinking。

### 6.2 `kimi-code-text`

调用 `kimi -p <prompt> --output-format text`。

根据 `kimi-code` 源码（`apps/kimi-code/src/cli/run-prompt.ts`），`text` 模式的输出规则如下：

- **stdout**：`PromptTranscriptWriter` 通过 `PromptBlockWriter` 写入 assistant 的最终回复，每个 block 开头带 `• `（`PROMPT_BLOCK_BULLET`），后续行缩进两个空格。
- **stderr**：thinking 同样通过 `PromptBlockWriter` 写入，因此也以 `• ` 开头；但 **tool call / tool result 在 `text` 模式下是 no-op**，只有 `tool.progress` 事件会直接原样写入 stderr（不带 bullet）。
...
- **缺点**：`text` 模式本身不输出 tool 名称和参数，只能拿到工具执行过程中的 progress 文本；thinking 与最终文本都带 `• `，需要按 stdout/stderr 分流。
...
|---|---|---|---|---|
| `kimi-code` | `kimi -p <prompt> --output-format stream-json [--session <id>]` | ⚠️ 事件级流式：assistant text 以单个 JSON 事件整段返回，不做逐字流式 | ❌ CLI 不暴露 | ✅ 精确事件 |
| `kimi-code-text` | `kimi -p <prompt> --output-format text [--session <id>]` | ✅ stdout 按字符/块流式 | ⚠️ 有局限：thinking 以 `• ` 引导的缩进块输出到 stderr，runner 已能完整捕获该块；但非 bullet 的 stderr 行仍要靠中英文启发式规则区分 thinking 与工具输出，可能误判，也无法还原具体 tool 名/参数 | ❌ 无法识别：不输出 tool 名称/参数，只能拿到 progress 原始文本 |
