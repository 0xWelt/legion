...
| `claude-code` | `claude -p`<br>✅ | `--resume <id>`<br>✅ | `--output-format stream-json --verbose`<br>⚠️ 为事件级流式：thinking 与 assistant text/tool 以完整 JSON 事件整段返回，非 token 级流式 | `--permission-mode bypassPermissions`<br>✅ | ✅ |
