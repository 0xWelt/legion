...
| `kimi-code-text` | `kimi -p`<br>✅ | `--session <id>`<br>✅ | `--output-format text`<br>⚠️ 1. stdout 按字符/块流式输出，但无法识别具体工具名/参数；2. stderr 中非 bullet 行是 tool.progress 原始文本，只能靠启发式规则与 thinking 区分，可能误判 | 同上<br>✅ | ❌ |
| `claude-code` | `claude -p`<br>✅ | `--resume <id>`<br>✅ | `--output-format stream-json --verbose`<br>✅ | `--permission-mode bypassPermissions`<br>✅ | ✅ |

## IM 支持矩阵

| 平台 | Workspace 映射 | Session 映射 | 原生 Slash Command | 消息编辑 | 输入中提示 | 流式回复 | 工具渲染 | 消息长度限制 |
|---|---|---|---|---|---|---|---|---|
| Discord | Text Channel | Channel / Thread | ✅ | ✅ | ✅ | 文本块级编辑 + debounce（⚠️ 2000 字符截断） | 独立消息（⚠️ 不折叠，多工具时较刷屏） | 2000 字符 |
| Lark（飞书） | Chat（群聊） | Chat / 回复消息 | ❌（文本命令解析，⚠️ 无原生 slash command） | ✅ | ❌（⚠️ 无 typing 提示） | 单张交互卡片全量更新（⚠️ 同 Chat 内只有一张汇总卡片，多 session 并行时互相覆盖） | 折叠面板 | 主文本 3000 / 面板 2000 |
